...
object MyImageClasses2 {


  val path = "features/my_classes_new3.csv"
  val path2 = "features/my_test_classes_new3.csv"

  val imageType: RDD[(String, (Int, Double))] = DatasetCreatorCollab.spark.textFile(path)
    .union(DatasetCreatorCollab.spark.textFile(path2))
    .map {
      x =>
        val split = x.split("\t")
  ...
  def apply(foldersUnknownFeedback: List[String],
    ...
    lazy val df = DatasetCreatorCollab.sqlContext.read.parquet(foldersUnknownFeedback: _*).rdd


    val imgToObjects: RDD[(String, List[Int])] = df.flatMap {
      x =>
        val object_ = x.getAs[Int]("instanceId_objectId")
        val imageId = if (x.isNullAt(x.fieldIndex("ImageId"))) None
        else x.getAs[mutable.WrappedArray[String]]("ImageId").toList.headOption
        imageId.map(_ -> List(object_))
  ...
  def ownerCat(foldersAll: List[String], dataset: RDD[Dataset]): RDD[Dataset] = {
    ...
    lazy val df = DatasetCreatorCollab.sqlContext.read.parquet(foldersAll: _*).rdd

    val imgToOwner: RDD[(String, List[Int])] = df.flatMap {
      x =>
        val owner = x.getAs[Int]("metadata_ownerId")
        val imageId = if (x.isNullAt(x.fieldIndex("ImageId"))) None
        else x.getAs[mutable.WrappedArray[String]]("ImageId").toList.headOption
        imageId.map(_ -> List(owner))
    }.distinct.reduceByKey(_ ::: _)
      .mapValues(_.distinct)
    ...
    val imgToUser: RDD[(String, List[Int])] = df.flatMap {
      x =>
        val user = x.getAs[Int]("instanceId_userId")
        val imageId = if (x.isNullAt(x.fieldIndex("ImageId"))) None
        else x.getAs[mutable.WrappedArray[String]]("ImageId").toList.headOption
        imageId.map(_ -> List(user))
    }.distinct.reduceByKey(_ ::: _)
      .mapValues(_.distinct)
  ...
  def userLikeCat(foldersWithFeedback: List[String], dataset: RDD[Dataset]): RDD[Dataset] = {
    ...
    lazy val df = DatasetCreatorCollab.sqlContext.read.parquet(foldersWithFeedback: _*).rdd.filter { x => x.getAs[mutable.WrappedArray[String]]("feedback").toList.contains("Liked") }
